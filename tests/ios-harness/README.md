...
- **Compare Tab (On-Device Parity Gate)**: 3-way comparison of Python reference / saved release baseline / current build using on-device noise-matched synthesis (see below)
...
./setup.sh /path/to/PocketTTS-v0.5.0.zip
```
...
└── PocketTTSDemo/
    ...
    └── PocketTTSDemo/
        ...
        ├── ContentView.swift        # Main UI (SwiftUI, tabbed)
        ├── ReferenceTestView.swift  # Compare tab (on-device parity gate)
        ...
        ├── ReferenceAudio/          # Committed parity-gate fixtures
        │   ├── manifest.json            # Phrase/reference metadata
        │   ├── reference_*_int16.wav    # Python reference audio (4 phrases)
        │   └── noise/                   # Captured noise tensors (.npy) for
        │                                #   noise-matched on-device synthesis
        └── Models/                  # Model files (folder reference)
            ├── model.safetensors        # Main FlowLM model (~225MB)
            ├── tokenizer.model          # SentencePiece tokenizer
            └── voices/
                └── alba.safetensors     # Voice file
```

`Models/` can hold the contents of **either** model directory — v1
(`kyutai-pocket-ios/`) or v2 (`kyutai-pocket-ios-en2026-04/`). The engine
auto-detects the voice-file format: v1 voices are embedding sequences
(`audio_prompt`, `[1, seq, 1024]`); v2 voices are precomputed transformer
KV-states. Don't mix voices from one generation with weights from the other.
...
| **ReferenceTestView** | `ReferenceTestView.swift` | Compare tab — on-device noise-matched parity testing against bundled references |
...
## Compare Tab (On-Device Parity Gate)

The **Compare** tab is the on-device numerical parity gate for releases. It
runs a 3-way comparison on each of the 4 canonical phrases:

1. **Python reference** — `ReferenceAudio/reference_*_int16.wav`, generated by
   the Python reference pipeline
2. **Saved release baseline** — audio captured from the previous release build
3. **Current build** — synthesized on-device, live

Synthesis uses `synthesize_noise_matched` (exposed over UniFFI) with the
bundled noise tensors in `ReferenceAudio/noise/` — the exact noise the Python
reference consumed — so any waveform difference measures implementation
divergence, not RNG differences.

**Pass criterion**: waveform correlation **1.0000 on all 4 phrases** against
the Python reference. This is the on-device counterpart of the host gate
(`.claude/skills/verify/run_baseline.sh`); v0.5.0 reads 1.0000 × 4 on-device.
